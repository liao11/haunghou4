#import "QueenProgressHUD.h"
@interface QueenProgressHUD ()
...
@end
@implementation QueenProgressHUD
#pragma mark -
...
- (void)setMode:(QueenProgressHUDMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    if (mode == QueenProgressHUDModeDeterminate) {
		if ([NSThread isMainThread]) {
			[self updateProgress];
			[self setNeedsDisplay];
		} else {
...
- (void)updateIndicators {
    ...
    else if (mode == QueenProgressHUDModeCustomView && self.customView != nil){
        self.indicator = self.customView;
    } else {
         UIView *backView = [[UIView alloc] initWithFrame:CGRectMake(0.0f, 0.0f, 30.0f, 30.0f)];
        self.indicator = backView;
        UIImageView *loadView = [[UIImageView alloc] initWithImage:[QueenUtils quImageWithName:@"Hud_Rotation"]];
        self.loadView = loadView;
        loadView.frame = backView.bounds;
        [backView addSubview:loadView];
        CABasicAnimation *rotationAnimation = [CABasicAnimation animationWithKeyPath:@"transform.rotation.z"];
        rotationAnimation.toValue = @(M_PI * 2);
        rotationAnimation.repeatCount = HUGE;
        rotationAnimation.duration = 1.0;
        [loadView.layer addAnimation:rotationAnimation forKey:@"KCBasicAnimation_Rotation"];
	}
...
+ (QueenProgressHUD *)QueenShowHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	QueenProgressHUD *hud = [[QueenProgressHUD alloc] initWithView:view];
	[view addSubview:hud];
	[hud show:animated];
	return hud;
}
+ (BOOL)JPBaoFu_HideHUDForView:(UIView *)view animated:(BOOL)animated {
	...
	for (UIView *v in [view subviews]) {
		if ([v isKindOfClass:[QueenProgressHUD class]]) {
			viewToRemove = v;
		}
	}
	if (viewToRemove != nil) {
		QueenProgressHUD *HUD = (QueenProgressHUD *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
	if (!view) {
		[NSException raise:@"QueenProgressHUDViewIsNillException" 
					format:@"The view used in the QueenProgressHUD initializer is nil."];
	}
...
- (id)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        self.animationType = QueenProgressHUDAnimationFade;
        self.mode = QueenProgressHUDModeIndeterminate;
        self.labelText = nil;
...
- (void)done {
    isFinished = YES;
    self.alpha = 0.0;
    if(delegate != nil && [delegate conformsToProtocol:@protocol(QueenProgressHUDDelegate)]) {
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0;
    if (animated && animationType == QueenProgressHUDAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0;
        if (animationType == QueenProgressHUDAnimationZoom) {
